  ...
  def draw(self, screen):
      ...
      for n, horse in enumerate(self.app._horses):
        ...
        if horse.hidden:
            ...
            if self.timerStarted == True:
              textsurface = self.myfont.render(str(round(self.endTime - time.time())), True, BLACK)
              screen.blit(textsurface, (cx - textsurface.get_size()[0]//2, y))
        else:
...
class CountDown(States):
  ...
  def startup(self):
    ...
    self.sprite = AnimatedSprite.AnimatedSprite(frames, animation_time=1)
    self.sprite.pause()
    self.timerStarted = False
    self.endTime = None
...
class Game(States):
  ...
  def startup(self):
    ...
    self.countdown_sprite = AnimatedSprite.AnimatedSprite(frames, animation_time=1)
    self.countdown_sprite.pause()
    self.timerStarted = False
    self.endTime = None
...
def start():
  ...
  app = HorseApp(fps=30, size=config.tracksize)
